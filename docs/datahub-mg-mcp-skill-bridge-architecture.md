...
its `content_digest` field omitted; for a packet, the complete `approval`
result object is also omitted because approval is a post-packet-digest
attestation object. For an artifact-evidence record, the digest target is that
record with only its `canonical_record_digest` field omitted; its
`payload_digest` and `byte_length` remain in the target so the record metadata
is bound to the exact payload. For a proof, the digest target is the proof
record identified by `record_id` with both `content_digest` and the entire
`digest` evidence object omitted. No digest target contains the digest value
it computes. The recorded value is lowercase `sha256:<64-lowercase-hex>`.
...
digest. The approval snapshot payload is a separate RFC 8785 JCS SHA-256
domain named `approval_snapshot_payload`; `approval_snapshot_digest` is its
digest and is computed from the immutable snapshot payload below. Approval is
...
The approval result attestation is a separate RFC 8785 JCS SHA-256 domain
named `approval_attestation`; `approval_attestation_digest` is its digest and
is never a substitute for the packet's `content_digest` or a proof's canonical
record digest. Approval result attestation fields are not recursive digest
inputs to the digest that names them: `approval_attestation_digest` is derived
...
`reviewer_identity`, `approved_at`, `disposition`, `approved_head_sha`,
`approval_attestation_digest`, and `approval_event_binding`; the nested
`packet_record_id` and `request_id` must resolve to the same approved packet
and request as the outer binding. A change to any outer mirror without a
corresponding attestation update is `PACKET_APPROVAL_MISMATCH`.
...
approval:
  ...
  approval_attestation:
    ...
    record_id: approval-attestation-<stable-id>
    packet_record_id: <packet-stable-id>
    request_id: <request-id>
    approved_content_digest: null
    approved_packet_digest: null
...
`approval_snapshot_digest` field from the digest target. The derived snapshot
is a post-packet-digest attestation projection rather than a packet digest
input.

```yaml
schema_name: governed_packet_approval_snapshot_payload
schema_version: "1.0"
packet_record_id: <packet-approved-id>
...
The digest target is this object with only `approval_snapshot_digest` omitted.
The digest algorithm is SHA-256 and the canonicalization is RFC 8785 JCS.
Approved and successor revisions bind to the same immutable snapshot domain,
and no approval binding may require the final packet `content_digest` as an
input.
...
artifact_evidence:
  - schema_name: governed_artifact_evidence
    ...
    artifact_outcome: produced|not_produced|skipped_existing
...
`required_artifacts` is a list of packet required-artifact declarations. Each
declaration has a stable `artifact_id`, the exact literal `artifact_path`, the
expected `expected_role`, and the media type when applicable. Each declaration
must bind to exactly one evidence record through `packet_required_artifact`,
and each `packet_required_artifact` in artifact evidence must equal that
declaration's `artifact_id`. `artifact_role` must equal the declaration's
`expected_role` before any role-specific provenance rule is applied. A
...
carried forward. An `executing` or `validated` wrapper must populate the
complete approval binding set from its immutable approved revision:
`approved_packet_record_id`, `approval_snapshot_digest`,
`approved_packet_digest`, `approved_content_digest`, and
`approval_attestation_digest`. A partially populated structured wrapper binding
is invalid. The approved revision's wrapper may use the sentinel
...
An executing or validated successor must also carry an
`authorized_scope_projection` inside `authorization_binding`. The projection
must equal the resolved approved revision's authorized scope field-for-field
before authorization carry-forward can pass. It includes objective,
source_mode, source_mode_policy, context record and digest, pre-retrieval
manifest binding, repository owner, name, base commit, branch, identity, path,
root, common directory, readable paths, writable paths, DataHub facts relied
on, MG MCP records relied on, authorized tools, denied tools, allowed commands,
required artifacts, validation requirements, stop_condition, expires_at, and
screening. Only successor-specific record identity, timestamps, status,
execution_status, supersedes, terminal worktree head, failures, and result
evidence may differ. Writable-path expansion, tool expansion, command
expansion, artifact substitution, validation weakening, stop-condition
weakening, or expiry extension invalidates carry-forward; a scope mismatch is
`PACKET_APPROVAL_MISMATCH` and fails both `authorization_carry_forward_validation`
and `lineage_authorization_validation`.
...
different. Each `packet_required_artifact` must equal its packet entry's
`artifact_id`, each `artifact_path` must be the same literal approved
repository path, and `artifact_role` must equal that packet declaration's
`expected_role` before role-specific provenance rules are applied. The
`artifact_outcome` is independent of the approved role and must be one of
`produced`, `not_produced`, or `skipped_existing`. `presence` must be
`present` or `not_produced`. A `present` artifact is read as bytes at
validation time; its record captures the exact non-null `byte_length` and
`payload_digest`, sets `nonproduction_reason: null` for `produced` evidence,
sets `nonproduction_reason` to a non-null attributable terminal cause for
`skipped_existing` evidence, and binds those values and its `lifecycle_stage`
into `canonical_record_digest`.
...
`presence: present` for every required artifact and rejects `not_produced` and
`skipped_existing` outcomes. A non-validated terminal profile may mix
`present` artifacts created before its terminal result with `not_produced`
artifacts that were never created; it must never fabricate a file or payload
digest. For `not_produced` evidence, the complete serialized field must be
`delta_provenance: null`; it must not serialize an object whose members have
null values. Any other presence/field combination is structurally invalid and
...
Allowed terminal status and execution-status pairs are profile-specific:

- `terminal_proposed_initial` and `terminal_proposed_refined`: `proposed/not_started`, `proposed/blocked`, or `proposed/failed` only.
- `terminal_approved`: `approved/approved`, `approved/blocked`, or `approved/failed` only.
- `terminal_executing`: `executing/executing`, `executing/blocked`, or `executing/failed` only.
- `terminal_validated`: `validated/validated` only.

No profile may claim a later stage than the revisions it actually contains.
Rejected, timed-out, and unknown outcomes require a fail-closed terminal pair
plus attributable failure, retrieval, or authority evidence. This table
defines all allowed wrapper-status and execution-status combinations for every
transition and terminal revision.
...
carry a carrier-only `authorization_binding` that identifies the approved
packet record ID, the immutable `approval_snapshot_digest`, its
`approved_content_digest`, its `approved_packet_digest`, and its
`approval_attestation_digest`. The `approve`-to-
`execute` and `execute`-to-`validate` edges verify this authorization carry-
forward through `authorization_carry_forward_validation`. Only mutation of the
approved revision itself or its approval payload invalidates approval;
conforming `1.1` successor creation does not. Packet schema `1.0` instead
invalidates approval on every successor creation and requires reapproval.
`proof.approval.approved_packet_record_id`, `approval_snapshot_digest`,
`approved_packet_digest`, `approved_content_digest`,
`approval_attestation_digest`, `approved_head_sha`,
`approval_event_binding`, and
`terminal_packet_record_id` capture this binding. The packet and proof
approval bindings must each carry the same `approval_snapshot_digest`,
`approved_packet_digest`, `approved_content_digest`,
...
`authorization_binding` digests must match those fields and its
`authorized_scope_projection` must match the resolved approved revision's
authorized scope field-for-field. For the approved revision itself, the
wrapper sentinel `authorization_binding: self` resolves to that revision's own
`record_id`, immutable `approval_snapshot_digest`, verified
`approval_digest_payload` digest, approved `approved_packet_digest`, approved
`approved_content_digest`, approved `approval_attestation_digest`, approved
`approved_head_sha`, `approval_event_binding`, and `content_digest`. The `self`
form is valid on the approved revision whether or not that revision is terminal
or has conforming successors. No resolved packet field or packet content
digest is used to carry or verify authorization binding.

The exact authorization-scope projection bound into an executing or validated
successor must contain `objective`, `source_mode`, `source_mode_policy`,
`context_record_id`, `context_content_digest`, `pre_retrieval_manifest_digest`,
`repository.owner`, `repository.name`, `repository.base_commit`,
`repository.branch`, `repository.worktree.identity`, `repository.worktree.path`,
`repository.worktree.repository_root`, `repository.worktree.common_dir`,
`repository.readable_paths`, `repository.writable_paths`, `datahub_facts_relied_on`,
`mg_mcp_records_relied_on`, `authorized_tools`, `denied_tools`, `allowed_commands`,
`required_artifacts`, `validation`, `stop_condition`, `expires_at`, and
`screening`. The only fields permitted to differ from the resolved approved
revision are the successor's own record identity, timestamps, `status`,
`execution_status`, `supersedes`, terminal worktree head, failures, and result
evidence. Any writable-path expansion, tool expansion, command expansion,
artifact substitution, validation weakening, stop-condition weakening, or
expiry extension invalidates carry-forward and returns `PACKET_APPROVAL_MISMATCH`.
...
approval payload and approval attestation. Its `node_identity` values must
...
`binds_approval_payload`, `binds_context`, `binds_packet`,
...
revision lineage, every artifact record, and, when the selected lineage profile
reaches approval, both approval carriers. Each edge is unique by
`(from_node_identity, to_node_identity, relationship)`, carries
`binding_validation`, and its relationship must be one of `binds_packet`,
`binds_context`, `binds_packet_revision_lineage`,
`binds_approval_attestation`, `binds_approval_payload`, or
`contains_artifact_evidence`. Nodes sort lexicographically by `node_identity`;
edges sort lexicographically by `from_node_identity`, `to_node_identity`, then
`relationship`. Noncanonical ordering fails with `PROOF_INCOMPLETE`.
...
10. Empty `artifact_evidence` passes only with an empty
  ...
  A validated proof may not use `not_produced` or `skipped_existing`; a record
  for bytes that exist may not use `not_produced`; unchanged existing bytes in
  nonvalidated terminal profiles may use `skipped_existing`. A required
  artifact without either truthful form of evidence, an invalid presence/role/
  outcome/provenance combination, a noncanonical collection order, an
  unresolved declared graph identity, or a graph node unreachable from the
  root returns `PROOF_INCOMPLETE`.
11. A pending `terminal_proposed_initial` or `terminal_proposed_refined`
  ...
  returns `PROOF_INCOMPLETE`. A validated proof with any `skipped_existing`
  evidence also returns `PROOF_INCOMPLETE`.
12. `execution_start_head_sha` is profile-conditional: it is null for
  ...
  before execution begins. The lifecycle-applicable baseline field is
  `pre_execution_baseline_head_sha` before execution begins and
  `execution_start_head_sha` after execution starts. A mismatch against the
  wrong lifecycle field returns `PROOF_INCOMPLETE` unless the underlying
  repository object lookup itself fails, in which case `WORKTREE_INVALID`
  remains the only allowed repository-object failure.
...
14. Every present `generated`, `modified`, or `validation_output` artifact passes
  ...
  bound repository evidence or the lifecycle-applicable baseline field returns
  `PROOF_INCOMPLETE`. Before execution begins, the lifecycle-applicable
  baseline field is `pre_execution_baseline_head_sha`; after execution starts,
  it is `execution_start_head_sha`. A supplied `base_payload_digest`,
  `result_payload_digest`, or `diff_digest` that is malformed or does not
  verify returns `DIGEST_INVALID`. `not_produced` or `validation_output`
  evidence containing a provenance object returns `PROOF_INCOMPLETE`.
  Produced generated evidence proves the artifact path is absent from the Git
  tree at `base_head_sha` and uses `base_payload_digest: null`. Produced
...
`human_approved`, `reviewer_identity`, `approved_at`, `disposition`,
...
computed over the resolved approved revision's `approval_digest_payload` only,
where the revision is resolved from `proof.approval.approved_packet_record_id`.
The verifier must not hash the complete approved revision record, the terminal
packet's payload, the record identified by `packet_binding.record_id`, or any
approval attestation field. Separately, `approval.approval_snapshot_digest`
must equal the RFC 8785 JCS SHA-256 digest computed over the deterministic
...
| Required artifact, artifact identity, media type, proof graph node, packet revision, edge, status, skipped-existing evidence, or `supersedes` evidence is missing or structurally invalid | `PROOF_INCOMPLETE` |
...
- **Approval:** use `APPROVAL_REQUIRED`, `APPROVAL_INVALID`, or
  ...
  timestamp, disposition, exact head binding, or reviewer-authenticated event
