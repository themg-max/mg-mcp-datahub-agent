...
| `local-oss` | **explicit** `--mode=local-oss` | Optional **live local DataHub OSS** path via official `mcp-server-datahub==0.6.0` over **HTTP** (`http://127.0.0.1:8000/mcp`) (Mode B). Fail-closed unless `DATAHUB_LOCAL_MCP_ALLOW=true`. Exactly one read-only `tools/call`. Authority `record_status=LOCAL_OSS_MCP_LIVE_READ`, `runtime_retrieval_status=VERIFIED_LOCAL_ONLY`. See [datahub-judge-quickstart.md](./datahub-judge-quickstart.md). |
